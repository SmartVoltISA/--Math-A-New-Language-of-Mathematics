...
## Quotient geometry
For an equivalence `~` and structural distance `d`, the construction

`d_Q([x],[y])=inf{d(x',y'):x'~x,y'~y}`

is only a **quotient-distance candidate** in general. Non-negativity and symmetry follow from the corresponding base properties, but separation and triangle inequality are not automatic for an arbitrary equivalence relation. The construction is promoted to a pseudometric/metric only after explicit compatibility/separation conditions or exhaustive verification on the declared finite domain. A standard sufficient case is an equivalence generated by a family of isometries, such as a group action by isometries.
